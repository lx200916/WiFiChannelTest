...
fun APScreen(paddingValues: PaddingValues, APInfo: JSONObject) {
    ...
    val apChartEntrys = remember(apInfo) {
        ...
        val latest_snapshot_time =
            (apTrends.getIntOpt("latest_snapshot_time") - (currentChannelUtilization.length() - 1) * 2 * 60) * 1000L

        val chartEntrys = listOf<MutableList<ChartEntry>>(
...
//    val iconMap = mapOf<String,>()
    ...
    Column(
        ...
        if (apChartEntrys != null) Card(
            ...
            ProvideChartStyle(m3ChartStyle(entityColors = LineColors).let {
                it.copy(
                    it.axis.copy(
                        axisLabelLineCount = 2,
                    )
                )
            }) {
                Chart(
